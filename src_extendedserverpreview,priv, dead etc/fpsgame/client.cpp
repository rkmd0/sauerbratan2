...
namespace game
{
    ...
    int extinfoplayerparser(ucharbuf p, struct extplayerdata& data)
    {
        ...
        strncpy(data.name, strdata, MAXEXTNAMELENGTH-1);
        data.name[MAXEXTNAMELENGTH-1] = 0;
        getstring(strdata, p); // team
        strncpy(data.team, strdata, MAXEXTTEAMLENGTH-1);
        data.team[MAXEXTTEAMLENGTH-1] = 0;
    ...
    int extinfoteamparser(ucharbuf p, int expectedteams, struct teamsinfo& data) {
        ...
        loopi(min(32,expectedteams)) {
            getstring(strdata, p);
            strncpy(td.teamname, strdata, MAXEXTTEAMLENGTH-1);
            td.teamname[MAXEXTTEAMLENGTH-1] = 0;
            td.score = getint(p);
            td.bases = getint(p);
            ...
            if(extinfotype == EXT_PLAYERSTATS) {
                ...
                loopi(lastpreviewdata.nplayers) {
                    ...
                    if(!isold) {
                        ...
                        strcpy(teamnames[nteams], lastpreviewdata.players[i].team);
                        nteams++;
                    }
                }
                loopi(nteams) {
    ...
    void drawgroup(g3d_gui *g, const char* name) {
        ...
        loopi(lastpreviewdata.nplayers) {
            if (isingroup(name, i)) {
                ...
                // Check the privilege level of the player
                int privilege = lastpreviewdata.players[i].privilege;
                if (privilege >= PRIV_ADMIN) {
                    color = 0xFF8000; // Admin color
                } else if (privilege >= PRIV_AUTH) {
                    color = 0xC040C0; // Authorized color
                } else if (privilege>= PRIV_MASTER) {
                    color = 0x40FF80; // Default color
                } else { color = 0xFFFFFF; }

                // Check if the player is dead and adjust color
                if (lastpreviewdata.players[i].state == CS_DEAD) {
                    color = (color >> 1) & 0x7F7F7F;
                }

                g->textf("%s ", color, NULL, lastpreviewdata.players[i].name);
            }
        }
        ...
        if(showaccuracy && !lastpreviewdata.tinfo.notteammode)
        {
            g->pushlist();
            g->strut(6);
            g->text("acc", 0xA0A0A0);
            loopi(lastpreviewdata.nplayers) {
                if(isingroup(name, i)) {
                    g->textf("%d", 0xFFFFDD, NULL, lastpreviewdata.players[i].acc);
                }
            }
            g->poplist();
        }

        if(hidefrags && !lastpreviewdata.tinfo.notteammode)
        {
            g->pushlist();
            g->strut(6);
            g->text("frags", 0xA0A0A0);
            loopi(lastpreviewdata.nplayers) {
                if(isingroup(name, i)) {
                    g->textf("%d", 0xFFFFDD, NULL, lastpreviewdata.players[i].frags);
                }
        ...
        if(showdeaths && !lastpreviewdata.tinfo.notteammode)
        {
            g->pushlist();
            g->strut(6);
            g->text("deaths", 0xA0A0A0);
            loopi(lastpreviewdata.nplayers) {
                if(isingroup(name, i)) {
                    g->textf("%d", 0xFFFFDD, NULL, lastpreviewdata.players[i].deaths);
                }
            }
            g->poplist();
        }

        if(showkpd && !lastpreviewdata.tinfo.notteammode)
        {
            g->pushlist();
            g->strut(6);
            g->text("frags", 0xA0A0A0);
            loopi(lastpreviewdata.nplayers) {
                if(isingroup(name, i)) {
                    g->textf("%d", 0xFFFFDD, NULL, lastpreviewdata.players[i].frags/lastpreviewdata.players[i].deaths);
                }
            }
            g->poplist();
        }
    ...
    void drawspecs(g3d_gui *g) {
        g->pushlist();
        g->strut(12);
        g->text("\f2spectator", 0xA0A0A0);
        loopi(lastpreviewdata.nplayers) {
            if (isspec(i)) {
                int privilege = lastpreviewdata.players[i].privilege;
                int color = (privilege >= PRIV_ADMIN) ? 0xFF8000 : (privilege >= PRIV_AUTH) ? 0xC040C0 : (privilege >= PRIV_MASTER) ? 0x40FF80 : 0xFFFFFF;
                color = (lastpreviewdata.players[i].state == CS_DEAD) ? ((color >> 1) & 0x7F7F7F) : color;
                g->textf("%s", color, NULL, lastpreviewdata.players[i].name);
            }
        }
        g->poplist();
    ...
    int showserverpreview(g3d_gui *g) {
        if(lastpreviewdata.hasserverdata) {
            ...
            if(enet_address_get_host_ip(&lastpreviewdata.servaddress, hostname, sizeof(hostname)) >= 0) {
                ...
                int clientcolor = lastpreviewdata.sdata.nclients>=lastpreviewdata.sdata.maxclients ? 0xFF4040 : 0xFFFFFF;
                int pingcolor = lastpreviewdata.sdata.ping > 99 ? 0xFFC040 : 0xFFFFFF;
                g->textf("%s", 0xFFFFFF, NULL, lastpreviewdata.sdata.description);
                noseperator ? g->space(1.5f) : g->separator();
                g->textf("%d/%d", clientcolor, NULL, lastpreviewdata.sdata.nclients, lastpreviewdata.sdata.maxclients);
                noseperator ? g->space(1.5f) : g->separator();
                g->textf("%dms", pingcolor, NULL, lastpreviewdata.sdata.ping);
                ...
                g->spring();*/
                noseperator ? g->space(1.5f) : g->separator();
                g->textf("%s", 0xFFFFFF, NULL, server::modename(lastpreviewdata.sdata.mode));
                noseperator ? g->space(1.5f) : g->separator();;
                g->textf("%s", 0xFFFFFF, NULL, lastpreviewdata.sdata.servname);
                if(lastpreviewdata.sdata.gamespeed != 100) {
                    noseperator ? g->space(1.5f) : g->separator();;
                    g->textf("%d.%02dx", 0xFFFFFF, NULL, lastpreviewdata.sdata.gamespeed/100, lastpreviewdata.sdata.gamespeed%100);
                }
                noseperator ? g->space(1.5f) : g->separator();
                int secs = lastpreviewdata.sdata.timelimit%60, mins = lastpreviewdata.sdata.timelimit/60;
                int clockcolor = mins < 1 ? gameclocklowcolour : gameclockcolour;
                g->pushlist();
                g->strut(mins >= 10 ? 4.5f : 3.5f);
                g->textf((mins == 0 && secs == 0) ? "Intermission" : "%d:%02d", clockcolor, NULL, mins, secs);
                g->poplist();
                if(lastpreviewdata.sdata.gamepaused) {
                    noseperator ? g->space(1.5f) : g->separator();
                    g->text("paused", 0xFFFFFF);
                }
                g->spring();
                g->poplist();
            }
        ...
        if(lastpreviewdata.hasplayerdata && lastpreviewdata.nplayers) {
            ...
            if(lastpreviewdata.tinfo.notteammode) {
                ...
                loopi(groups) {
                    ...
                    if((k%2)==0) g->pushlist();
                    g->pushlist();
                    if(lastpreviewdata.tinfo.teams[i].score>=10000) g->textf("%s: WIN", (strcmp(lastpreviewdata.tinfo.teams[i].teamname, "good") == 0) ? 0x6496FF : 0xFF4B19 , NULL, lastpreviewdata.tinfo.teams[i].teamname);
                    else g->textf("%s: %d", (strcmp(lastpreviewdata.tinfo.teams[i].teamname, "good") == 0) ? 0x6496FF : 0xFF4B19 , NULL, lastpreviewdata.tinfo.teams[i].teamname, lastpreviewdata.tinfo.teams[i].score);
                    g->pushlist();
                    drawgroup(g, lastpreviewdata.tinfo.teams[i].teamname); //0x6496FF  : 0xFF4B19
                    g->poplist();
                    g->poplist();
        ...
        g->pushlist();
        g->spring();
        if(g->button("back", 0xFFFFDD, "arrow_bw")&G3D_UP) {
            g->poplist();
            lastpreviewdata.reset();
            return -1;
        }
        noseperator ? g->space(1.5f) : g->separator();
        if(g->button("connect", 0xFFFFDD, "action")&G3D_UP) {
            g->poplist();
            g->end();
            lastpreviewdata.reset();
            return 1;
    ...
    extern void add_client_name_history(const char *name);
    ...
    void switchname(const char *name)
    {
        ...
        add_client_name_history(player1->name);
    }
    void printname()
    {
    ...
    COMMAND(sayteam, "C");
        ...
        void mocksay(char *text) {
            char mockedText[MAXTRANS];
            bool upper = true; // Start with uppercase

                for (int i = 0; text[i]; i++) {
                    if (isalpha(text[i])) {
                        if (upper) {
                            mockedText[i] = toupper(text[i]);
                        } else {
                            mockedText[i] = tolower(text[i]);
                        }
                ...
                mockedText[strlen(text)] = '\0';
                toserver(mockedText);
            }

            COMMANDN(mocksay, mocksay, "C");

        void mocksayteam(char *text) {
            char mockedText[MAXTRANS];
            bool upper = true; // Start with uppercase
    ...
    COMMANDN(mocksayteam, mocksayteam, "C");
