...
#define MAXEXTTEAMLENGTH 6


#include "weaponstats_type.h"

struct teamdata {
    char teamname[MAXEXTTEAMLENGTH];
    int score;
    int bases;
    teamdata() {
        teamname[0] = 0;
    ...
    void update(const struct teamdata& ndata) {
        copystring(teamname, ndata.teamname, MAXEXTTEAMLENGTH-1);
        teamname[MAXEXTTEAMLENGTH-1] = 0;
        score = ndata.score;
        bases = ndata.bases;
    }
...
#define MAXEXTNAMELENGTH 17
struct extplayerdata {
    int cn;
    int ping;
    char name[MAXEXTNAMELENGTH];
    char team[MAXEXTTEAMLENGTH];
    int frags;
    int flags;
    int deaths;
    int teamkills;
    ...
    void update(const struct extplayerdata& ndata) {
        cn = ndata.cn;
        ping = ndata.ping;
        copystring(name, ndata.name, MAXEXTNAMELENGTH-1);
        name[MAXEXTNAMELENGTH-1] = 0;
        copystring(team, ndata.team, MAXEXTTEAMLENGTH-1);
        team[MAXEXTTEAMLENGTH-1] = 0;
        frags = ndata.frags;
        flags = ndata.flags;
        deaths = ndata.deaths;
...
#define SERVUPDATEINTERVAL 1000
...
namespace game
{
    ...
    extern int showaccuracy;
    extern int hidefrags;
    extern int showdeaths;
    extern int showkpd;
    extern int showflags;
    extern int noseperator;
    extern int statuscolor(fpsent *d, int color);
    extern int gameclockcolour;
    extern int gameclocklowcolour;
