...
namespace game
{
    ...
    void renderscoreboard(g3d_gui &g, bool firstpass)
    {
        ...
        if(showservinfo && address)
        {
            ...
            {
                if(servinfo[0]) g.titlef("%.25s", alternativescoreboard ? textcolor : 0xFFFF80, NULL, servinfo);
        ...
        g.pushlist();
        g.spring();
        g.text(server::modename(gamemode), alternativescoreboard ? textcolor : 0xFFFF80);
        g.separator();
        const char *mname = getclientmap();
        g.text(mname[0] ? mname : "[new map]", alternativescoreboard ? textcolor : 0xFFFF80);
        extern int gamespeed;
        if(gamespeed != 100) { g.separator(); g.textf("%d.%02dx", alternativescoreboard ? textcolor : 0xFFFF80, NULL, gamespeed/100, gamespeed%100); }
        if(m_timed && mname[0] && (maplimit >= 0 || intermission))
        {
            g.separator();
            if(intermission) g.text("intermission", alternativescoreboard ? textcolor : 0xFFFF80);
            else
            {
                ...
                g.textf("%d:%02d", alternativescoreboard ? textcolor : 0xFFFF80, NULL, mins, secs);
                g.poplist();
            }
        }
        if(ispaused()) { g.separator(); g.text("paused", alternativescoreboard ? textcolor : 0xFFFF80); }
        g.spring();
        g.poplist();
            ...
            if(showclientnum || player1->privilege>=PRIV_MASTER)
            {
                ...
                loopv(spectators)
                {
                    ...
                    /*if (showspecicons) {
                        ...
                        g.text(colorname(o), statuscolor(o, 0xFFFFDD), "spectator");
                    } else if (showplayericon == 0) {
                        g.text(colorname(o), statuscolor(o, 0xFFFFDD), NULL);
                    }*/
