...
char *loadAndCleanFile(const char *fn, size_t *size, bool utf8)
{
    stream *f = openfile(fn, "rb");
    if (!f) return NULL;
    stream::offset fsize = f->size();
    ...
    // Clean up the loaded data by removing empty lines and non-printable characters
    char *cleaned = new char[len + 1];
    size_t cleanedlen = 0;
    bool prevCharWasPrintable = false;
    ...
    for (size_t i = 0; i < len; ++i)
    {
        ...
        if (currentChar == '\n')
        {
            if (!prevCharWasPrintable)
            {
                // Skip this newline if the previous character was non-printable
                continue;
            }
            else
            {
                prevCharWasPrintable = false;
            }
        }
        else if (isprint(currentChar) && !isspace(currentChar))
        {
            cleaned[cleanedlen++] = currentChar;
            prevCharWasPrintable = true;
        }
        else if (prevCharWasPrintable)
        {
            cleaned[cleanedlen++] = ' '; // Replace non-printable characters and spaces with a space
            prevCharWasPrintable = false;
        }
    }
